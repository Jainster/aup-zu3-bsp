...
#ifndef _hw_joystick_slice_0_core_h_
#define _hw_joystick_slice_0_core_h_

#include <systemc.h>
#include "properties.h"


class hw_joystick_slice_0_core : public sc_module 
{
    public: 
       hw_joystick_slice_0_core (sc_core::sc_module_name nm, const xsc::common_cpp::properties& props)
            : sc_module(nm)
              , Din    ( "Din" )
              , Dout   ( "Dout" )
    {
        SC_HAS_PROCESS(hw_joystick_slice_0_core);
        SC_METHOD(get_slice_value);
        sensitive << Din;

    }

    virtual ~hw_joystick_slice_0_core() = default;
